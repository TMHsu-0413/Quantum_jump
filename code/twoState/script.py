...
def compile_and_run(input_file,threshold):
    try:
        ...
        subprocess.run(
            ["g++", "--std=c++17", "RLBSP/RLBSP.cpp", "-o", "output/RLBSP"]
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python3 script.py <number of node> <number of node> ..."
        )
        sys.exit(1)
    ...
    for i in range(1,len(sys.argv)):
        ...
        subprocess.run(["python","main.py","graph.txt",number_of_nodes,"10","14","0.4","0.85","1"])
        ...
        graph.different_threshold([list(rLbsp["Probability"][number_of_nodes]),list(qPath["Probability"][number_of_nodes]),list(qLeap["Probability"][number_of_nodes])],threshold,number_of_nodes)
        graph.execution_time_on_different_threshold([rLbsp["Time"][number_of_nodes],qPath["Time"][number_of_nodes],qLeap["Time"][number_of_nodes]],number_of_nodes,threshold)
