...
class qLeap {
public:
  qLeap(string name, double th, double beta)
      : filename(name), threshold(th), beta(beta) {
    ;
  }
  void run() {
...
private:
  ...
  vector<vector<vector<purifyLabel>>>
      purifyTable; // {fidelity, fidelity improve, prob}
    ...
    for (int i = 0; i < n; i++) {
      for (int j = i + 1; j < n; j++) {
        ...
        for (int k = 1; k <= min(memory[i], memory[j]); k++) {
          ...
          purifyTable[i][j].push_back(
              {curFidelity, curFidelity - purifyTable[i][j][k - 1].fidelity,
               curProb});
        }
      }
    }
    ...
    for(int i=0; i<n; i++){
        for(int j=i+1; j<n; j++){
            ...
            for(int k=0; k<purifyTable[i][j].size(); k++){
                cout<<purifyTable[i][j][k].fidelity<<"
    "<<purifyTable[i][j][k].fidelity_improvement<<"
    "<<purifyTable[i][j][k].prob<<endl;
            }
        }
    }
    */
  ...
  bool purifyDecision() {
    ...
    for (int i = 0; i < l - 1; i++) {
      ...
      while (purifyTime[i] < purifyTable[curNode][nxtNode].size() &&
             (purifyTable[curNode][nxtNode][purifyTime[i]].fidelity <
              fidelity_avg)) {
        ++purifyTime[i];
      }
  ...
  bool checkPath() {
    ...
    // 起點終點的memory檢查
    if ((memory[s] < (purifyTime[0] + 1)) ||
        (memory[d] < (purifyTime[l - 2] + 1)))
      return false;
    // 中間的memory檢查
    for (int i = 1; i < l - 1; i++) {
