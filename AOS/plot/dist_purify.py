...
class ChartGenerator:
    ...
    def __init__(self, dataName, Ylabel, Xlabel):
       ...
       ##data start##
        ...
        linestyle = ["-","--",":"]
        for i in range(numOfAlgo):
            ax1.plot(x_data, y[i], color = color[i], lw = 2.5, linestyle = linestyle[i], marker = marker[i], markersize = 15, markerfacecolor = "none", markeredgewidth = 2.5, zorder=i)
        # plt.show()

        plt.xticks(fontsize = Xticks_fontsize,rotation=90)
        ...
        plt.savefig(directory_path + 'avg_purifyTime_dis.eps'.format(pdfName)) 
        plt.savefig(directory_path2 + 'avg_purifyTime_dis.jpg'.format(pdfName)) 
